...
export function ActionTable({ actions }: { actions: InventoryAction[] }) {
  ...
  return (
    <div className={styles.wrapper}>
      <p className={styles.hint}>Compare the queue in the selected sort order. Expand a product to see its recommendation and stock projection. Scroll sideways on smaller screens.</p>
      <div className={styles.scroll} tabIndex={0} role="region" aria-label="Inventory action comparison">
        <table className={styles.table}>
          ...
          <tbody>{actions.map((action, index) => {
            ...
            const key = `${action.status}-${action.sku_id}`;
            ...
            return <Fragment key={key}>
              <tr>
                <th scope="row" className={styles.product}>
                  ...
                  <span className={styles.statusLine}><span className={`pill pill-${action.status}`}>{metrics.historyReview ? "Review history" : statusLabel[action.status]}</span><span className={styles.secondary}>Priority {number(metrics.priority)}</span></span>
                </th>
                <td className={styles.numeric}>{number(metrics.stock, " units")}</td>
                <td className={styles.numeric}>
                  <strong>{metrics.historyReview ? "Needs history" : number(metrics.coverage, " days")}</strong>
                ...
                <td className={styles.numeric}>{metrics.reorderUnits === null ? action.status === "urgent" ? "Unavailable" : "—" : number(metrics.reorderUnits, " units")}<span className={styles.secondary}>{action.status === "urgent" ? "Suggested replenishment" : "No reorder suggested"}</span></td>
                <td className={styles.numeric}>{metrics.impact === null ? "Not established" : currencyFormatter.format(metrics.impact)}<span className={styles.secondary}>{action.status === "urgent" ? "Estimated profit at risk" : "Capital tied up"}</span></td>
                <td><span className={action.data_quality_confidence === "low" ? styles.risk : undefined}>{confidenceLabel[action.data_quality_confidence]}</span>{action.data_quality_warnings.length > 0 ? <span className={styles.secondary}>{action.data_quality_warnings.length} {action.data_quality_warnings.length === 1 ? "warning" : "warnings"} in details</span> : null}</td>
