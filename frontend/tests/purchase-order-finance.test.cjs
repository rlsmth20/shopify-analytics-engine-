...
function load(file, dependencies = {}, extraSource = "", globals = {}) {
  ...
  const context = { exports: {}, Number, Math, Date, Map, Set, Intl, ...globals, require(name) {
...
function renderPurchaseOrder(draft, expanded = true) {
  let hook = 0;
  const calls = [];
  const stateUpdates = [];
  ...
  walk(tree);
  return { calls, window, buttons, elementsById, stateUpdates };
}
