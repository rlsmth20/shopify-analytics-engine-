    ...
    {data && <>
      ...
      <div className={styles.columns}>
        <section className={styles.card} aria-labelledby="capacity-title"><div className={styles.cardHeading}><h2 id="capacity-title">First-contact capacity</h2><span className={styles.tag}>Rolling 24 hours</span></div>
          {capacity ? <><div className={styles.capacityNumber}><strong>{number(capacity.used)}<span> / {number(capacity.limit)}</span></strong><span>{number(capacity.remaining)} available</span></div>
            ...
            <div className={styles.callout}><strong>{capacity.remaining > 0 ? "A ceiling, never a quota." : "Capacity reached. The growth loop continues."}</strong><p>Send only when a prospect qualifies. Replies to engaged merchants, research, learning, and funnel work continue at the ceiling.</p></div>
